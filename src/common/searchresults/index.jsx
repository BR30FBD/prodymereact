  ...
  const [categoryList,setCategoryList]=useState([])
    ...
    const handleClose = () => setOpen(false);
     ...
     const handlecategory=(e)=>{
      ...
      nav("/searchresults",{state:{id:"1",data:res.data,category:e,product:""}})
      ...
      const handletags=()=>{
        ...
        if(res.data.length>0){
          nav("/searchresults",{state:{id:"1",data:res.data,category:location.state.category,product:tags}})
      window.location.reload()
...
<div className='empty-box-wrapper-footer'></div>
     ...
     <div className='bredcrumb'>
        ...
        <p className='label-form h3-form' style={{fontSize:"28px"}}>Results for <strong>'{location.state.category || ""}, {location.state.product}' </strong></p>
   <div className='blog-container h3-font'>
    <h1  style={{color:"#FF7A34",borderBottom:"2px solid #FF7A34"}}>Products</h1>
    <h1  >Blogs</h1>
    <h1  >Services</h1>

   </div>
   <hr style={{background:"#FF7A34",borderBottom:"1px solid #FF7A34"}} />
     </div>
  ...
  <span className="ant-input-suffix" style={{position:"absolute",marginLeft:"200px"}}>
    ...
    >
      ...
      >
        ...
        <div>
          <h2 className="product-heading h3-font">Category of {location.state.category || ""}</h2>
...
<input type="checkbox" className='box-value' style={{marginLeft:"26px"}} id={item.product_id} onClick={(e)=>handlecart(e)}/><h5 className='h3-font' >Add To Cart</h5>
      ...
      <StarResults fivestar={fivestar} threestar={threestar}/>
        </div>
    </div>
    </div>
  )
